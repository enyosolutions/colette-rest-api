...
namespace  App\Services\AutoAB\Internal\AB;
...
class Node extends \Twig_Node
{
    ...
    public function compile(\Twig_Compiler $compiler)
    {
        ...
        $compiler->write("else if (isset(\$_COOKIE['__ab_" . $this->test_name . "'])) {\n");
        $compiler->indent()->write("foreach (\$ab_variants as \$ab_variant_iteration) {\n");
        $compiler->indent()->write("if (\$ab_variant_iteration['name'] === \$_COOKIE['__ab_" . $this->test_name . "']) {\n");
        $compiler->indent()->write("\$ab_selected_test = \$ab_variant_iteration;\n");
        $compiler->write("break;\n");
        $compiler->outdent()->write("}\n");
        $compiler->outdent()->write("}\n");
        ...
        $compiler->write("if (!isset(\$_COOKIE['__ab_" . $this->test_name . "'])) {\n");
        $compiler->indent()->write("setcookie('__ab_"  . $this->test_name . "', \$ab_selected_test['name'], time()+60*60*24*3 );\n\n");
        $compiler->outdent()->write("}\n\n");
        // Execute the variant
